...
/** Generates unique variable names within a method body.
  *
  * Shared across all nested [[BlockBuilder]]s so names are unique across the
  * whole method, not just within each block.
  *
  * Given hint `"result"`, successive calls produce: `result`, `result1`,
  * `result2`, …
  */
@Singleton
class NameGenerator:
