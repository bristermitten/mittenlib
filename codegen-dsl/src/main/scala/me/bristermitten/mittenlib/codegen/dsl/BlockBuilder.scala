 ...
 *
 * If both branches are [[Terminated]], the result is [[Terminated]].
 * Otherwise it degrades to [[Open]] — you can't be certain which branch runs.
 *
 * This is the Scala 3 approximation of a Haskell type class instance selection:
 * {{{
 ...
 *
 *   - **Context functions** (`BlockBuilder ?=> Block[S]`) make the builder available
 *     as a given inside lambdas — you write `return_(x)` instead of `b.return_(x)`.
 *   - **`let`** returns `Block[S]` where `S` is inferred from the body's return type.
 *     One method handles both terminating and open cases — no `letOpen` split.
 *   - **`ifThenElse`** uses the [[Merge]] match type: when both branches are
 *     `Block[Terminated]`, the return type is `Block[Terminated]` — no Optional.
 *
 * Usage:
 * {{{
...
class BlockBuilder(private val names: NameGenerator):
   ...
   */
  def let[S](tpe: TypeRef, value: Expr)(body: Var => BlockBuilder ?=> Block[S]): Block[S] =
    let(tpe, None, value)(body)

  def let[S](tpe: TypeRef, hint: String, value: Expr)(body: Var => BlockBuilder ?=> Block[S]): Block[S] =
    let(tpe, Some(hint), value)(body)

  private def let[S](tpe: TypeRef, hint: Option[String], value: Expr)
                    (body: Var => BlockBuilder ?=> Block[S]): Block[S] =
    checkOpen()
  ...
  /**
   * Declare a variable without a scoping lambda (flat style).
   * Prefer [[let]] for correctness — use this when multiple variables must
   * be declared before a natural termination point.
   */
   ...
   * - Both branches `Block[Terminated]` → returns `Block[Terminated]` (no Optional!)
   * - Otherwise → returns `Block[Open]`
   *
   * This is the Scala 3 type-class approximation — the compiler resolves [[Merge]]
   * at the call site based on the inferred branch types.
   */
  def ifThenElse[A, B](cond: Expr)
                      (thenBody: BlockBuilder ?=> Block[A])
  ...
  def forEach(tpe: TypeRef, iterable: Expr, hint: Option[String] = None)
             (body: Var => BlockBuilder ?=> Unit): Unit =
    checkOpen()
  ...
  def tryCatch(exType: TypeRef, hint: Option[String] = None)
    ...
    stmts += Statement.TryCatch(tryBuilder.buildOpen(), exType, exVar, catchBuilder.buildOpen())
  ...
  private def checkOpen(): Unit =
    if done then throw IllegalStateException(
      s"Cannot add statements after block termination (${stmts.size} statements accumulated)"
    )

object BlockBuilder:
  /** Entry point: build a method body. The builder is available as a context parameter. */
  ...
  def return_(value: Expr)(using b: BlockBuilder): Block[Terminated] = b.return_(value)
  def returnVoid()(using b: BlockBuilder): Block[Terminated] = b.returnVoid()
  def throw_(expr: Expr)(using b: BlockBuilder): Block[Terminated] = b.throw_(expr)

  def let[S](tpe: TypeRef, value: Expr)(body: Var => BlockBuilder ?=> Block[S])(using b: BlockBuilder): Block[S] = b.let(tpe, value)(body)
  def let[S](tpe: TypeRef, hint: String, value: Expr)(body: Var => BlockBuilder ?=> Block[S])(using b: BlockBuilder): Block[S] = b.let(tpe, hint, value)(body)

  def declare(tpe: TypeRef, value: Expr)(using b: BlockBuilder): Var = b.declare(tpe, value)
  def declare(tpe: TypeRef, hint: String, value: Expr)(using b: BlockBuilder): Var = b.declare(tpe, hint, value)

  def assign(target: Var, value: Expr)(using b: BlockBuilder): Unit = b.assign(target, value)
  def assignTo(target: Expr, value: Expr)(using b: BlockBuilder): Unit = b.assignTo(target, value)
  ...
  def ifThen(cond: Expr)(body: BlockBuilder ?=> Unit)(using b: BlockBuilder): Unit = b.ifThen(cond)(body)
  def ifThenElse[A, B](cond: Expr)(thenBody: BlockBuilder ?=> Block[A])(elseBody: BlockBuilder ?=> Block[B])(using b: BlockBuilder): Block[Merge[A, B]] = b.ifThenElse(cond)(thenBody)(elseBody)

  def forEach(tpe: TypeRef, iterable: Expr, hint: Option[String] = None)(body: Var => BlockBuilder ?=> Unit)(using b: BlockBuilder): Unit = b.forEach(tpe, iterable, hint)(body)
  def tryCatch(exType: TypeRef, hint: Option[String] = None)(tryBody: BlockBuilder ?=> Unit)(catchBody: Var => BlockBuilder ?=> Unit)(using b: BlockBuilder): Unit = b.tryCatch(exType, hint)(tryBody)(catchBody)
