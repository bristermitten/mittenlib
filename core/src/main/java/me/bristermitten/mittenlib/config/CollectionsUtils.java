...
public class CollectionsUtils {
    private static final TypeToken<List<DataTree>> LIST_MAP_STRING_OBJECT_TOKEN =
            new TypeToken<List<DataTree>>() {
            };
    ...
    public static <T> Result<List<T>> deserializeList(
        ...
        Result<List<DataTree>> rawListRes =
                baseContext.getMapper().map(rawData, LIST_MAP_STRING_OBJECT_TOKEN);
        return rawListRes.flatMap(f -> deserialiseListFrom(f, baseContext, deserializationFunction));
    }

    private static <T> Result<List<T>> deserialiseListFrom(
     ...
     * @param rawData                 the raw data to deserialise, which should represent a {@code Map<String,
     *                                Object>}
     * @param baseContext             the base context to use for deserialization
     ...
     * @return a {@link Result} containing the deserialised map, or a {@link Result#fail(Exception)}
     * if deserialisation failed
     */
    public static <K, V> Result<Map<K, V>> deserializeMap(
        ...
        Result<Map<K, DataTree>> rawMapRes =
                baseContext
                        .getMapper()
                        .map(
                                rawData,
                                (TypeToken<Map<K, DataTree>>)
                                        TypeToken.get(
                                                new GenericParameterizedType(
                                                        Map.class, keyType, MAP_STRING_OBJECT_TOKEN.getType())));
        return rawMapRes.flatMap(
                rawMap -> {
                    ...
                    for (Map.Entry<K, DataTree> entry : rawMap.entrySet()) {
                        Result<V> deserialised =
                                deserializationFunction.apply(baseContext.withData(entry.getValue()));
                        deserialised.error().ifPresent(errors::add);
    ...
    static class GenericParameterizedType implements ParameterizedType {
        ...
        @Contract(pure = true)
        public GenericParameterizedType(
                @NotNull Class<?> container, @NotNull Type @NotNull ... wrapped) {
