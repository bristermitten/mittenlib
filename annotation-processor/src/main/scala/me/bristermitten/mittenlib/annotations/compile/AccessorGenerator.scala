...
/** Generates accessor methods for configuration classes. This class creates
  * getter methods for fields and "with" methods that act as immutable setters,
  * returning new instances with modified values. It also handles method
  * overriding and annotation preservation when generating accessor methods.
  */
class AccessorGenerator @Inject() (
  ...
  /** Creates a getter method for a field.
    ...
    *   The builder for the type spec
    * @param element
    *   The variable element
    * @param field
    *   The field spec
    */
  def createGetterMethod(
    ...
    builder.addAnnotation(
      AnnotationSpec
        .builder(classOf[Contract])
        .addMember("pure", CodeBlock.of("true"))
        .build()
    )
    typeSpecBuilder.addMethod(builder.build())

  /** Creates a getter method that overrides an existing method. This method
    * preserves annotations from the original method (except for private
    * annotations) and adds appropriate nullability and contract annotations.
    *
    * @param typeSpecBuilder
    *   The builder for the type spec
    * @param overriding
    *   The executable element being overridden
    * @param fromField
    *   The field spec that the getter will return
    */
  def createGetterMethodOverriding(
    ...
    val builder = MethodSpec
      .methodBuilder(overriding.getSimpleName.toString)
      .addJavadoc(
        ...
        case _ =>
          ...
          ast.source() match {
            case classSource: ConfigTypeSource.ClassConfigTypeSource
                if classSource.parent().isPresent =>
          ...
          typeSpecBuilder.addMethod(
            withMethodBuilder
              .addStatement(
                "return new $T(" + constructorParams + ")",
                configImplClassName
              )
              .build()
          )
  ...
  private def getFieldAccessorName(variableElement: VariableElement): String =
    methodNames.safeMethodName(
      variableElement,
      variableElement.getEnclosingElement.asInstanceOf[TypeElement]
    )
