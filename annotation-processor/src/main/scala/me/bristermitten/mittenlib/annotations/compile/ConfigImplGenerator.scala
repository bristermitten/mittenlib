...
import me.bristermitten.mittenlib.annotations.ast.{ASTSettings, AbstractConfigStructure, ConfigTypeSource, Property}
...
class ConfigImplGenerator @Inject() (
  ...
  def emit(ast: AbstractConfigStructure): JavaFile =
    ...
    val source = TypeSpec.classBuilder(configImplClassName)
      .addJavadoc(
        """Generated data implementation of {@link $T}.
          |""".stripMargin, ast.source().element())

    emitInto(ast, source)

    JavaFile.builder(configImplClassName.packageName(), source.build())
   ...
   * class.
   */
  private def emitInto(ast: AbstractConfigStructure, source: TypeSpec.Builder): Unit =
    val configImplClassName = configurationClassNameGenerator.generateConfigurationClassName(ast.source().element())
  ...
  private def makeAbstractIfUnion(ast: AbstractConfigStructure, source: TypeSpec.Builder): Unit =
    ast match {
   ...
   */
  private def addSourceElement(ast: AbstractConfigStructure, builder: TypeSpec.Builder): Unit =
    if (ast.settings().source() != null) {
      val publicClassName = configurationClassNameGenerator.getPublicClassName(ast)
      val implementationClassName = configurationClassNameGenerator.translateConfigClassName(ast)

      val configFieldBuilder = FieldSpec.builder(
          ParameterizedTypeName.get(ClassName.get(classOf[Configuration[?]]), publicClassName), "CONFIG")
   ...
   */
  private def addInheritance(ast: AbstractConfigStructure, source: TypeSpec.Builder): Unit =
    ast.source() match {
   ...
   */
  private def addGeneratedConfigAnnotations(ast: AbstractConfigStructure, source: TypeSpec.Builder): Unit =
    val uninitializableProperties = ast.properties().asScala
    ...
    val generatedConfigBuilder = AnnotationSpec.builder(classOf[GeneratedConfig])
      .addMember("source", "$T.class", ast.name())
      .addMember("isDynamicallyInitializable", "$L", Boolean.box(configStructureAnalysis.isDynamicallyInitializable(ast)))
    ...
    source.addAnnotation(AnnotationSpec.builder(classOf[Generated])
   ...
   */
  private def addNestedClassModifiers(ast: AbstractConfigStructure, source: TypeSpec.Builder): Unit =
   ...
   */
  private def addProperties(ast: AbstractConfigStructure, source: TypeSpec.Builder): Unit =
  ...
  private def addStandardObjectMethods(
    ...
    if (ast.settings().generateToString()) {
      val toString = toStringGenerator.generateToString(ast.properties(), configImplClassName)
   ...
   */
  private def addChildClasses(ast: AbstractConfigStructure, source: TypeSpec.Builder): Unit =
    for (child <- ast.enclosed().asScala) {
      val childClassName = configurationClassNameGenerator.translateConfigClassName(child)
  ...
  private def addProperty(property: Property, source: TypeSpec.Builder): Unit =
    ...
    property.source() match {
      case fieldSource: Property.PropertySource.FieldSource =>
        accessorGenerator.createGetterMethod(source, fieldSource.element(), field)
      case methodSource: Property.PropertySource.MethodSource =>
        accessorGenerator.createGetterMethodOverriding(source, methodSource.element(), field)
    }
   ...
   */
  private def addAllArgsConstructor(source: TypeSpec.Builder, ast: AbstractConfigStructure): Unit =
  ...
  private def buildSuperConstructorParams(
    ...
    val parentParams = parentConfig.properties().asScala
   ...
   */
  private def addPropertyParameters(ast: AbstractConfigStructure, constructor: MethodSpec.Builder): Unit =
    for (property <- ast.properties().asScala) {
  ...
  private def addInnerDefaultMethodImpl(
    ...
    ast.source() match {
      case _: ConfigTypeSource.InterfaceConfigTypeSource =>
        val hasAnyDefaultValue = ast.properties().asScala.exists(_.settings().hasDefaultValue())
        if (!hasAnyDefaultValue) {
          JOptional.empty()
        } else {
          val concreteConfigClassName = configurationClassNameGenerator.getConcreteConfigClassName(ast)
          val innerName = configurationClassNameGenerator.getDefaultMethodAccessClassName(ast)
          ...
          for (property <- ast.properties().asScala if !property.settings().hasDefaultValue()) {
            innerBuilder.addMethod(MethodSpec.methodBuilder(property.name())
              .addModifiers(Modifier.PUBLIC)
