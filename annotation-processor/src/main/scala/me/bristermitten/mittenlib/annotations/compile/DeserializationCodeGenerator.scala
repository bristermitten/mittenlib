...
import me.bristermitten.mittenlib.annotations.compile.deserializer.{GenericTypeDeserializerGenerator, NonGenericTypeDeserializerGenerator}
...
class DeserializationCodeGenerator @Inject() (
  ...
  def createDeserializeMethodFor(
    ...
    val elementResultType = configurationClassNameGenerator.publicPropertyClassName(
    ...
    if (isGenericType && typeElementOpt.isPresent) {
      genericTypeDeserializerGenerator.generateDeserializeMethod(
        propertyAST, property, dtoType, elementType, wrappedElementType, typeElementOpt.get(), elementResultType, daoName,
        fieldNameGenerator, methodNames
      )
    } else {
      nonGenericTypeDeserializerGenerator.generateDeserializeMethod(
        propertyAST, property, dtoType, elementType, wrappedElementType, elementResultType, daoName,
        fieldNameGenerator, methodNames
      )
    }
