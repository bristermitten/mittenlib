...
import _root_.me.bristermitten.mittenlib.annotations.ast.{AbstractConfigStructure, CustomDeserializerInfo, Property}
...
import _root_.me.bristermitten.mittenlib.annotations.util.{NewtypeUtil, TypesUtil}
...
import _root_.me.bristermitten.mittenlib.config.tree.{DataTree, DataTreeTransforms}
...
class NonGenericTypeDeserializerGenerator @Inject() (
  ...
  private def getDeserializationFunction(
    ...
    if (info.isStatic) {
      Expr.staticCall(TypeRef.of(ClassName.get(info.deserializerClass)), "deserialize",
        context.withData(withDataExpression)
      )
    } else {
      val fieldName = Strings.uncapitalize(info.deserializerClass.getSimpleName.toString)
      val provider = StagedExpr.field[java.util.function.Function[DeserializationContext, Result[?]]](
        Types.Function(Types.DeserializationContext, Types.Result(Types.Object)),
        fieldName
      )
      provider(context.withData(withDataExpression))
    }

  def dataTreeConvert(tpe: TypeName, dataTreeType: TypeName, value: Expr): Expr =
    val targetType = if (tpe.isBoxedPrimitive) tpe.unbox() else tpe
    if (dataTreeType == ClassName.get(classOf[DataTree.DataTreeLiteral.DataTreeLiteralInt])) {
    ...
    if (dataTreeType == ClassName.get(classOf[DataTree.DataTreeLiteral.DataTreeLiteralFloat])) {
  ...
  def handleNonGenericType(
    ...
    if (treeTypeOpt.isPresent) {
      ...
      ifThen(fm.instanceOf(treeType)) {
        val convert = dataTreeConvert(safeType, treeTypeOpt.get(), (~fm.cast(treeType)).as[DataTree].value.cast(TypeRef.of(getLiteralValueType(safeType))))
        return_(ResultExpr.ok(convert))
      }
    }
    
    // 3.3 Custom Deserializers (no fallback)
    val customDeserializerOptional = customDeserializers.getCustomInfo(property.propertyType())
    
    if (customDeserializerOptional.isPresent && !customDeserializerOptional.get().isFallback) {
      return_(getDeserializationFunction(context, customDeserializerOptional.get(), DataTreeExpr.loadFrom(fm)))
      ...
      val stringLitType = TypeRef.of(classOf[DataTree.DataTreeLiteral.DataTreeLiteralString])
      ...
      val loaderFieldName = configurationClassNameGenerator.getDeserializerProviderFieldName(elementType)
      ...
      ifThen(fm.instanceOf(mapDataType)) {
        ...
        val provider = StagedExpr.field[com.google.inject.Provider[java.util.function.Function[DeserializationContext, Result[?]]]](
          Types.Provider(Types.Function(Types.DeserializationContext, Types.Result(Types.Object))),
          loaderFieldName
        )
      ...
      val element = elementType.asInstanceOf[DeclaredType].asElement().asInstanceOf[TypeElement]
      ...
      val implClass = typeName match {
        case p: ParameterizedTypeName => ParameterizedTypeName.get(implClassBase, p.typeArguments.toArray(new Array[TypeName](0)) *)
        case _ => implClassBase
      }
      val typeTokenT = TypeRef.of(classOf[TypeToken[?]])(TypeRef.of(TypeName.get(boxedUnderlying)))
      val lambdaParam = Var("val", TypeRef.of(TypeName.get(boxedUnderlying)))
      
      return_(
        context.getMapper.map(
          fm,
          Expr.newAnonymous(typeTokenT)
        ).mapResult(
          Expr.lambdaExpr(
    ...
    } else if (customDeserializerOptional.isPresent && customDeserializerOptional.get().isFallback) {
      return_(getDeserializationFunction(context, customDeserializerOptional.get(), DataTreeExpr.loadFrom(fm)))
    } else {
      val useObjectMapperSerialization = typesUtil.getAnnotation(property.source().element(), classOf[UseObjectMapperSerialization])
      if (useObjectMapperSerialization != null) {
        val propertyTypeName = TypeRef.of(configurationClassNameGenerator.publicPropertyClassName(property))
        val dataTreeTransforms = TypeRef.of(classOf[DataTreeTransforms])
        return_(
          context.getMapper.map(
            Expr.staticCall(dataTreeTransforms, "toPOJO",
              DataTreeExpr.loadFrom(fm)
            ),
            Expr.staticCall(Types.TypeToken, "get", Expr.staticField(propertyTypeName, "class"))
          )
        )
      } else {
        if (property.settings().hasDefaultValue()) {
          ...
          ifThen(!fm.instanceOf(dataTreeType)) {
            return_(
              ResultExpr.fail(
                Expr.staticCall(TypeRef.of(classOf[ConfigLoadingErrors]), "invalidPropertyTypeException",
  ...
  private def addEnumDeserialisation(
    ...
    val enumValue = property.settings().enumParsingScheme() match
      case EnumParsingSchemes.EXACT_MATCH =>
        declare(safeTypeRef, "enumValue",
          Expr.staticCall(TypeRef.of(classOf[Enums]), "valueOfOrNull",
      ...
      case EnumParsingSchemes.CASE_INSENSITIVE =>
        declare(safeTypeRef, "enumValue",
          Expr.staticCall(TypeRef.of(classOf[Enums]), "valueOfIgnoreCase",
            cv.cast(Types.String),
            Expr.staticField(safeTypeRef, "class")
          )
        )
    ...
    ifThen(ev.isNull) {
      return_(
        ResultExpr.fail(
          Expr.staticCall(TypeRef.of(classOf[ConfigLoadingErrors]), "invalidEnumException",
            Expr.staticField(safeTypeRef, "class"),
            Expr.str(property.name()),
            fm
          )
