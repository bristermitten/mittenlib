...
import java.util.{ArrayList => JArrayList, LinkedHashMap => JLinkedHashMap, LinkedHashSet => JLinkedHashSet, List => JList, Map => JMap, Optional => JOptional, Set => JSet}
import javax.annotation.processing.Generated
import javax.lang.model.element.{Modifier, TypeElement}
import javax.lang.model.`type`.TypeMirror
import me.bristermitten.mittenlib.annotations.ast.{AbstractConfigStructure, ConfigTypeSource, CustomDeserializerInfo, Property}
...
import me.bristermitten.mittenlib.annotations.util.{ConfigStructureAnalysis, TypesUtil}
...
class ConfigLoaderGenerator @Inject() (
  ...
  def emit(ast: AbstractConfigStructure): JavaFile =
    ...
    JavaFile.builder(loaderClassName.packageName(), builder.build())
   ...
   */
  private def createLoaderBuilder(ast: AbstractConfigStructure): TypeSpec.Builder =
    val loaderClassName = classNameGenerator.getDeserializerClassName(ast)
    val publicClassName = classNameGenerator.getPublicClassName(ast)

    val builder = TypeSpec.classBuilder(loaderClassName.simpleName())
      .addJavadoc(
   ...
   */
  private def addChildLoaderClasses(ast: AbstractConfigStructure, loaderBuilder: TypeSpec.Builder): Unit =
   ...
   */
  private def addFieldsAndConstructor(ast: AbstractConfigStructure, builder: TypeSpec.Builder): Unit =
    ...
    for (property <- ast.properties().asScala) {
      ...
      collectConfigTypes(property.propertyType(), injectedTypes, injectedFieldNames)
    }
  ...
  private def collectConfigTypes(
    ...
    if (typesUtil.isConfigType(tpe)) {
      ...
      val providerType = ParameterizedTypeName.get(ClassName.get(classOf[Provider[?]]), subLoaderName)
  ...
  private def collectCustomDeserializers(
    ...
    customDeserializers.getCustomInfo(tpe).ifPresent(info => {
      if (!info.isStatic) {
        val deserializerClass = info.deserializerClass()
        val fieldName = Strings.uncapitalize(deserializerClass.getSimpleName.toString)
  ...
  private def addApplyMethod(
    ...
    val applyMethod = MethodSpec.methodBuilder("apply")
      .addJavadoc(
        """Deserializes the configuration from the given {@link $T}.
          ...
          |""".stripMargin, classOf[DeserializationContext], classOf[Result[?]])
    ...
    val dslBlock = BlockBuilder.buildOpen {
      ...
      tryCatch(Types.Exception) {
        ast match {
          case union: AbstractConfigStructure.Union =>
            for ((alternative, idx) <- union.alternatives().asScala.zipWithIndex) {
              val loaderFieldName = classNameGenerator.getDeserializerProviderFieldName(alternative.source().element().asType())
              val varName = s"var$idx"
              val alternativePublicType = classNameGenerator.getPublicClassName(alternative)
              val resultType = Types.Result(TypeRef.of(WildcardTypeName.subtypeOf(alternativePublicType)))
              val varRes = declare(resultType, s"${varName}Res",
                Expr.This.field(loaderFieldName).call("get").call("apply", context)
              )
            ...
            return_(Expr.staticCall(Types.Result, "fail", Expr.staticCall(TypeRef.of(classOf[ConfigLoadingErrors]), "noUnionMatch")))

          case _ =>
            val hasAnyDefault = ast.properties().asScala.exists(_.settings().hasDefaultValue())
            ...
            if (superClass.isDefined) {
              ...
              val superLoaderFieldName = classNameGenerator.getDeserializerProviderFieldName(parentType)
              val superPublicType = classNameGenerator.publicPropertyClassName(parentType)

              val varRes = declare(Types.Result(TypeRef.of(superPublicType)), s"${varName}Res",
                Expr.This.field(superLoaderFieldName).call("get").call("apply", context)
              )
              ...
              val parentVar = declare(TypeRef.of(superPublicType), varName, varRes.call("getOrThrow"))
            ...
            for ((property, idx) <- ast.properties().asScala.zipWithIndex) {
              ...
              val deserializeMethodArguments: List[Expr] = if (daoName != null && property.settings().hasDefaultValue()) {
                List(context, daoVarOpt.get)
              } else {
                List(context)
              }

              val varRes = declare(TypeRef.of(returnType), s"${varName}Res",
                Expr.This.call(deserializeMethod.name, deserializeMethodArguments *)
              )
              ...
              val propVar = declare(TypeRef.of(innerType), varName, varRes.call("getOrThrow"))
              constructorVars.add(propVar)
            }

            val constructorCall = Expr.new_(TypeRef.of(implClassName), constructorVars.asScala.toList *)

            if (configStructureAnalysis.needsValidation(ast)) {
              return_(
                Expr.staticCall(Types.Result, "ok", constructorCall.cast(TypeRef.of(publicClassName)))
                  .call("flatMap", Expr.methodRef(Expr.This.field("validator"), "validate"))
              )
            } else {
              return_(
                Expr.staticCall(Types.Result, "ok", constructorCall.cast(TypeRef.of(publicClassName)))
