...
import me.bristermitten.mittenlib.annotations.ast.{AbstractConfigStructure, CustomSerializerInfo, Property}
...
import me.bristermitten.mittenlib.config.{CollectionsUtils, SerializationContext}
...
class SerializationCodeGenerator @Inject() (
  ...
  def isSerializationSupported(ast: AbstractConfigStructure): Boolean =
    ast.properties().asScala.forall(property => !propertyIsUnserializable(property))

  def getUnsupportedSerializationProperties(ast: AbstractConfigStructure): java.util.List[String] =
  ...
  private def propertyIsUnserializable(property: Property): Boolean =
    ...
    if (typesUtil.getAnnotation(property.source().element(), classOf[UseObjectMapperSerialization]) != null) {
      return false
    }
    ...
    if (typesUtil.isNewtype(propertyTypeMirror)) {
      val underlyingType = typesUtil.getNewtypeUnderlyingType(propertyTypeMirror)
    ...
    if (wrappedType.hasTypeArguments) {
      ...
      if (typesUtil.isCollection(propertyTypeMirror) || canonicalName == classOf[java.util.Map[?, ?]].getName) {
  ...
  def addSerializeMethodsToSaver(typeSpecBuilder: TypeSpec.Builder, ast: AbstractConfigStructure): Unit =
    for (property <- ast.properties().asScala) {
  ...
  private def getSerializeParameterType(property: Property): TypeName =
    val typeName = configurationClassNameGenerator.publicPropertyClassName(property)
    typeName match
      case parameterizedTypeName: ParameterizedTypeName =>
        val rawType = parameterizedTypeName.rawType()
        if (rawType == ClassName.get(classOf[java.util.List[?]]) ||
            rawType == ClassName.get(classOf[java.util.Set[?]]) ||
            rawType == ClassName.get(classOf[java.util.Map[?, ?]])) {
          val typeArguments = parameterizedTypeName.typeArguments().asScala.map { arg =>
  ...
  private def createSerializeMethodFor(property: Property): MethodSpec =
    ...
    val useObjectMapper = typesUtil.getAnnotation(property.source().element(), classOf[UseObjectMapperSerialization]) != null
    ...
    val builder = MethodSpec.methodBuilder(methodName)
      .addJavadoc(
        """Serializes the {@code $L} property into a {@link $T}.
          ...
          |""".stripMargin, property.name(), classOf[DataTree]
      )
    ...
    val dslBlock = BlockBuilder.buildOpen {
      ...
      if (useObjectMapper) {
        return_(Expr.staticCall(Types.DataTreeTransforms, "loadFrom",
          context.call("getMapper").call("map", value)
        ))
      } else {
        val result = declare(Types.DataTree, "result", generateSerialization(property.propertyType(), value, 0))
        return_(result)
      }
    }
  ...
  private def generateSerialization(
    ...
    if (customSerializerOptional.isPresent) {
      ...
      val publicTypeName = configurationClassNameGenerator.publicPropertyClassName(tpe)
      val context = Var("context", Types.SerializationContext)
      if (info.isStatic) {
        Expr.staticCall(TypeRef.of(ClassName.get(info.serializerClass)), "serialize",
          input.cast(TypeRef.of(publicTypeName)),
          context
        )
      } else {
        val fieldName = Strings.uncapitalize(info.serializerClass.getSimpleName.toString) + ConfigurationClassNameGenerator.PROVIDER_SUFFIX
        Expr.This.field(fieldName).call("get").call("apply",
          input.cast(TypeRef.of(publicTypeName)),
          context
        )
      }
    ...
    // Newtype
      ...
      val typeElement = tpe.asInstanceOf[DeclaredType].asElement().asInstanceOf[TypeElement]
      val underlyingType = typesUtil.getNewtypeUnderlyingType(tpe)
      val accessor = NewtypeUtil.getAccessorName(typeElement).replaceAll("\\(\\)", "")
      val unwrappedVarName = input match {
        case Var(name, _) => s"${name}_${depth}_unwrapped"
        case _ => s"value_${depth}_unwrapped"
      }
      val unwrappedVar = declare(TypeRef.of(TypeName.get(underlyingType)), unwrappedVarName, input.call(accessor))
      generateSerialization(underlyingType, unwrappedVar, depth + 1)
    }
    // Config type
    else if (typesUtil.isConfigType(tpe)) {
      val saverFieldName = configurationClassNameGenerator.getSerializerProviderFieldName(tpe)
      val publicTypeName = configurationClassNameGenerator.publicPropertyClassName(tpe)
      val context = Var("context", Types.SerializationContext)
      Expr.This.field(saverFieldName).call("get").call("apply",
        input.cast(TypeRef.of(publicTypeName)),
        context
      )
    }
    // Generic collections (List, Map, Set)
      ...
      val canonicalName = wrappedType.erasure().getQualifiedName()
      val context = Var("context", Types.SerializationContext)
      if (canonicalName == classOf[java.util.List[?]].getName || canonicalName == classOf[java.util.Set[?]].getName) {
        val elementType = wrappedType.getTypeArguments.get(0)
        val serializeHelper = if (canonicalName == classOf[java.util.List[?]].getName) "serializeList" else "serializeSet"
        ...
        val lambdaBlock = BlockBuilder.build {
          val elementTarget = declare(Types.DataTree, s"res$depth", generateSerialization(elementType, elVar, depth + 1))
          return_(elementTarget)
        }

        Expr.staticCall(Types.CollectionsUtils, serializeHelper,
          input, context, Expr.Lambda(scala.List(elVar, ctxVar), lambdaBlock)
        )
        ...
        val lambdaBlock = BlockBuilder.build {
          val valTarget = declare(Types.DataTree, s"mapVal$depth", generateSerialization(valueType, valVar, depth + 1))
          return_(valTarget)
        }

        Expr.staticCall(Types.CollectionsUtils, "serializeMap",
          input, context, Expr.Lambda(scala.List(valVar, ctxVar), lambdaBlock)
        )
      } else {
    ...
    // Enum type
      ...
      Expr.staticCall(Types.DataTreeTransforms, "loadFrom",
