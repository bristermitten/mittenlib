 ...
 *
 * <ol>
 *   <li>Direct Type Match (short-circuits if object already matches target type)
 *   <li>{@link DataTree} Type Match (converts literal {@link DataTree} values to
 *       primitives/strings)
 *   <li>Custom Deserializers ({@link CustomDeserializerFor}) without {@link Fallback}
 *   <li>Enums
 *   <li>Config Types
 *   <li>Custom Deserializers ({@link CustomDeserializerFor}) with {@link Fallback}
 *   <li>Invalid Property / Object Mapper Fallback, either throwing an error or trying the {@link
 *       ObjectMapper} if nothing else works
 * </ol>
 */
public class NonGenericTypeDeserializerGenerator {
    ...
    /**
     * Generates a short-circuit when a property has a default value and the raw input value is
     * already of the expected target type. Specifically, if the property has a default value we
     * generate:
     *
     * <pre>{@code
    ...
    /**
     * Generates type checking and conversion logic when the raw input value is wrapped in a {@link
     * DataTree} literal (e.g. integer, float, string) that matches a primitive/string target.
     * Generates the code:
     *
     * <pre>{@code
    ...
    /**
     * Generates recursive deserialization logic for nested configurations by casting the property
     ...
     */
    private boolean handleInvalidPropertyType(
            MethodSpec.Builder builder,
            Property property,
            TypeElement dtoType,
            TypeMirror elementType,
            String fromMapName) {
        var useObjectMapperSerialization =
                typesUtil.getAnnotation(property.source().element(), UseObjectMapperSerialization.class);
        if (useObjectMapperSerialization != null) {
            ...
            builder.addStatement(
                    "return context.getMapper().map($T.toPOJO($T.loadFrom($L)), $T.get($T.class))",
        ...
        builder.addStatement(
                "return $T.fail($T.invalidPropertyTypeException($T.class, $S, $S, $L))",
                Result.class,
