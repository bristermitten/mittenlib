...
public class ConfigLoaderGenerator {
    ...
    private TypeSpec.Builder createLoaderBuilder(AbstractConfigStructure ast) {
        ...
        TypeSpec.Builder builder =
                TypeSpec.classBuilder(loaderClassName.simpleName())
        ...
        final List<MethodSpec> deserializeMethods =
                ast.properties().stream()
                        .map(
                                property ->
                                        deserializationCodeGenerator.createDeserializeMethodFor(
                                                dtoType, ast, property, daoName))
                        .toList();
    ...
    private void addFieldsAndConstructor(AbstractConfigStructure ast, TypeSpec.Builder builder) {
        ...
        if (ast.needsValidation()) {
            ...
            builder.addField(
                    FieldSpec.builder(
                                    validatorClassName, validatorFieldName, Modifier.PRIVATE, Modifier.FINAL)
                            .build());
        ...
        if (ast instanceof AbstractConfigStructure.Union union) {
            for (AbstractConfigStructure alternative : union.alternatives()) {
                collectConfigTypes(
                        alternative.source().element().asType(), injectedTypes, injectedFieldNames);
            }
        }
    ...
    private void collectConfigTypes(
        ...
        if (typesUtil.isConfigType(type)) {
            ...
            TypeName providerType =
                    ParameterizedTypeName.get(ClassName.get(Provider.class), subLoaderName);
     ...
     * properties.
     *
     * <p>Generates a recursive {@code flatMap} chain:
     *
     * <pre>{@code
    ...
    private void addApplyMethod(
        ...
        if (ast instanceof AbstractConfigStructure.Union union) {
            ...
            for (AbstractConfigStructure alternative : union.alternatives()) {
                ...
                deserialiseBuilder.add(
                        "this.$L.get().apply(context).map($T.class::cast).orElse(() -> \n",
                        loaderFieldName,
                        publicClassName);
        ...
        if (superClass.isPresent()) {
            String superLoaderFieldName =
                    classNameGenerator.getDeserializerProviderFieldName(superClass.get());
            expressionBuilder.add(
                    "this.$L.get().apply(context).flatMap(var$L -> \n", superLoaderFieldName, i++);
        }
