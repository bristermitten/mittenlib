...
public class ConfigSaverGenerator {
    ...
    private TypeSpec.Builder createSaverBuilder(AbstractConfigStructure ast) {
        ...
        TypeSpec.Builder builder = TypeSpec.classBuilder(saverClassName)
                .addJavadoc(
                        "Serializer/Saver implementation for {@link $T}.\n"
        ...
        applyMethod.addStatement(
                "$T<$T, $T> map = new $T<>()", Map.class, DataTree.class, DataTree.class, LinkedHashMap.class);
        ...
        MethodSpec.Builder method = MethodSpec.methodBuilder("generateDefault")
                .addJavadoc(
                        "Generates a default {@link $T} representation of the configuration,\n"
                                + "populating default values using method defaults or fallbacks.\n"
                                + "\n"
                                + "@param context the serialization context\n"
        ...
        for (Property property : ast.properties()) {
            ...
            if (property.settings().hasDefaultValue()) {
                ...
                if (property.settings().isNullable()) {
                    method.addStatement("map.put($T.string($S), $T.null_())", DataTree.class, key, DataTree.class);
                } else {
                ...
                if (wrapped.hasTypeArguments()) {
                    ...
                    if (canonicalName.equals(List.class.getName())) {
                        method.addStatement("map.put($T.string($S), $T.array())", DataTree.class, key, DataTree.class);
